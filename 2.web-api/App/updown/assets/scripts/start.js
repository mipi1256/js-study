...
/*
  ...
  선언된 함수들, 클래스를, 변수들을 모듈화 시켜서 객체 형태로 전달할 때는
...
export default function gameStart() {
  ...
  const makeIcon = () => {
    ...
    for(let n=1; n<=100; n++){
      // <div class="icon">1</div> THIS IS WHAT WE WANT TO SEE
  ...
  $numbers.onclick = e => {
    ...
    // 아이콘 아닌 곳을 클릭하면 이벤트가 발생하지 않도록.
    if(!e.target.matches('#numbers .icon')) return;
    
    // console.log(`사용자가 클릭한 아이콘 번호: ${e.target.dataset.iconNumber}번`); 확인

    // 사용자가 클릭한 아이콘의 솟자를 answer에 저장 (대소비교를 위해 정수로 변환)
