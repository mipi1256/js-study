...
const todos = [{
    id: 1,
    text: '할 일 1',
    done: false // checkbox를 클릭해서 할 일을 마쳐는지의 여부
  },
  {
    id: 2,
    text: '할 일 2',
...
function makeNewTodoNode(newTodo) {
  ...
  // 클래스 이름을 도개 이상 add하실 때는 각각 지정해야 합니다.
  ...
  const $ul = document.querySelector('.todo-list').appendChild($li);
...
function makeNewId() {
  if (todos.length > 0) { // If there are items in the todo list then the number will be different
    // 배열 내의 할일 객체 중 마지막 객체의 id보다 하나 크게.
    return todos[todos.length - 1].id + 1;
  } else { // If there no other todo list so the first input will be 1
...
function changeCheckState($label) {
  ...
  */
  ...
  //     todos.dataset.done.value = true;
  //    } else {
  //     todos.dataset.done.value = false;
  //    }    
  // }
...
function removeTodoData($delTarget) {
  ...
  // My try to answer the problem
  // const remData = +$delTarget.parentNode.dataset.id;
  // for(let i=0; i<todos.length; i++){
  //   if(remData === todos[i].id) {
  //     todos[i].id.splice([i], 1);
  //   }
  //   console.log(todos);
  //   // $delTarget.parentNode.removeChild($delTarget);
  // }
...
function enterModifyMode($modSpan) {
  ...
  // input 태그에는 .modify-input을 추가하고시고, input에는 기존의 할 일 텍스트가 미리 작성되어 있어야 합니다.
  const $label = $modSpan.parentNode.previousElementSibling;
  const $textSpan = $label.lastElementChild;

  const $modInput = document.createElement('input');
  // $modInput.setAttribute('type', 'text'); No need to write it because the default type is text / HOWEVER IF YOU WANT TO CHANGE THE TYPE THAT IS NOT AN TEXT THEN YOU NEED TO WRITE IT DOWN!!!
  $modInput.classList.add('modify-input');
  $modInput.setAttribute('value', $textSpan.textContent); // 기존 할 일 text를 input에 미리 세팅. 

  $label.replaceChild($modInput, $textSpan);

  // $modSpan.parentNode.parentNode.innerHTML = `<input class = "mod-input"> ${$modSpan} </input>`;
...
(function () {
  ...
  $todoList.addEventListener('click', e => {
    if (!e.target.matches('.todo-list .remove span')) {
      return; // remove에서만 이벤드가 동작하도록 처리.
    }
